...
glm = xff(['RA_PTSD_SV_day1day2_resp_corrected_PC.glm']);

% number of subjects
NrOfSub = glm.NrOfSubjects;

% number of GLM predictors (excluding constant?)
NrOfCond = glm.NrOfSubjectPredictors - 1; % minus constant

% number of ROI
NrOfVOI = 1;

% display contrast names. Useful for defining contrast
Predictor = glm.Predictor;
for i = 1:NrOfCond
    display(Predictor(i).Name2)
end
...
% covariates
covar = xff('COV_48subj_all.mat');
% check covariate names
names = covar.Info.names;
names{37}
%select the covariate
caps_total = covar.Data(:,37);

groups = {'PTSD', [105, 1074, 110, 119, 1205, 1232, 1237, 1245, 125, 1280, 1285, 1350, 45, 82, 85, 87, 88, 93, 98];...
    'Control', [1063, 1069, 1072, 115, 1206, 1208, 1244, 1266, 1273, 1284, 1291, 1304, 1305, 1309, 1340, 1344, 1345, 1346, 30, 38, 53, 56, 58, 60, 75, 83, 95, 96, 99]};
...
for i = 1:3
    ...
    vmp = glm.RFX_tMap(C, struct('subsel', setdiff(1:NrOfSub, i)','groups', groups));
    % Save the output vmp (optional). If you do save it, make sure to include
    % what contrast this is and which subject is left out
    vmp.SaveAs(['RA_PTSD_SV_day1day2_resp_corrected_PC_con_ls' int2str(i) 'o.vmp']);
end
vmp.ClearObject;
...
for i = 1:NrOfSub
    ...
    % leave out for this iteration of the loop
    % Compute a random-effects map for all but one subject
    vmp = glm.RFX_rMap(C, caps_total, struct('subsel', setdiff(1:NrOfSub, i)'));
    % Save the output vmp (optional). If you do save it, make sure to include
    % what contrast this is and which subject is left out
    vmp.SaveAs(['RA_PTSD_SV_day1day2_resp_corrected_PC_capstotal_ls' int2str(i) 'o.vmp']);
end
vmp.ClearObject;

vmp = glm.RFX_rMap(C, caps_total, setdiff(1:NrOfSub, 2)');
vmp.SaveAs(['test.vmp']);
